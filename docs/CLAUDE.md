...
토큰과 프리미티브는 전부 `app/globals.css` 한 곳에 있다. 화면 코드에는 색·크기
리터럴을 쓰지 않는다.

### 절대 하지 말 것

**숫자 팔레트를 직접 쓰지 않는다.** `slate-700`, `bg-black/10` 같은 것을 쓰면
그 자리에서만 다크 모드가 깨진다. 토큰 이름(`text-ink`, `border-hairline` …)을
쓰면 라이트/다크 두 블록이 알아서 갈아 끼운다. 유일한 예외는 모달 스크림
(`bg-black/50`)이고 이건 사양 그대로다.

**`.t-*` 는 utilities 레이어에 둔다.** components 에 두면 `.field` / `.btn-sm` 이
선언한 font-size 와 명시도가 같아 소스 순서상 프리미티브가 이긴다. 실제로
`field t-caption-sm` 이 13px 이 아니라 16px 로 렌더된 적이 있다.

**임의 크기를 적지 않는다.** `text-[17px]` 이나 `text-sm` 을 쓰면 화면마다 위계가
어긋난다. 아래 유틸에서 고른다. 이름은 참고 문서의 typography 토큰과 1:1 이라
문서와 대조할 수 있다.

| 유틸 | 값 | 쓰는 곳 |
|---|---|---|
| `.t-rating` | 64/700 | 활자만으로 위계를 세우는 단 한 자리 (목표 상세 진행률) |
| `.t-display-xl` | 28/700 | 페이지 제목 |
| `.t-display-lg` | 22/500 | 상세 화면 제목 (목록보다 조용하다) |
| `.t-display-sm` | 20/600 | 섹션 제목 |
| `.t-title-md` / `.t-title-sm` | 16/600, 16/500 | 카드 제목 |
| `.t-body-md` / `.t-body-sm` | 16/400, 14/400 | 본문 / 카드 메타 |
| `.t-caption` / `.t-caption-sm` | 14/500, 13/400 | 폼 라벨 / 부가 설명 |
| `.t-micro-label` | 12/700 | 아주 작은 라벨 |
| `.t-link` / `.t-nav-link` | 14/400, 16/600 | 인라인 링크 / 내비게이션 |

**spacing 을 토큰으로 만들지 말 것.** `--spacing-sm` 같은 이름은 Tailwind 의
`max-w-sm` · `w-md` 와 같은 네임스페이스를 쓴다. 실제로 그걸 노출했더니
`max-w-sm` 이 24rem 이 아니라 8px 로 해석돼 로그인 카드가 64px 로 찌그러졌다.
Tailwind 기본 스케일이 이미 문서와 같은 픽셀을 낸다 (0.5=2 · 2=8 · 6=24 · 16=64).

### 어휘

| 클래스 | 쓰는 곳 |
|---|---|
| `.card-liftable` | 끌거나 눌러서 다루는 것. hover 에 그림자가 뜬다 (할일 카드, 목표/계획 카드) |
| `.well` | "여기에 놓는다" 는 뜻이 필요한 영역만 (칸반 열, 연결 계획 미리보기) |
| `.rule` | 목록 행 구분선. 박스 대신 이걸로 나눈다 |
| `.btn` `.btn-primary/secondary/ghost` `.btn-sm` | 버튼. 새로 조합하지 않는다 |
| `.field` | 입력·셀렉트 |
| `.badge` + `-neutral/-primary/-luxe/-danger` | 상태 표시 |

### 지켜야 하는 두 가지

**강조색은 하나다.** Rausch(`#ff385c`)는 CTA·활성 메뉴·완료·진행률에서만 쓴다. Luxe
(`--luxe`)는 이월 배지 하나에 예외적으로 빌려 쓴 것이고, 여기서 더 늘리면
"대부분이 캔버스와 잉크" 라는 전제가 무너진다. 새 상태에 색을 주고 싶으면
먼저 `badge-neutral` 로 되는지 본다.

**그림자 단계는 하나다** (`--shadow-float`). 깊이는 그림자를 쌓아서가 아니라
면 분리와 헤어라인으로 만든다.

**문서에 있어도 이 앱에 표면이 없으면 넣지 않는다.** 참고 문서의 legal-link(법적
문구 밴드), error-hover(오류 링크), plus(Plus 서브브랜드), rounded.xl(카테고리
스트립)은 대응하는 화면이 없어 두지 않았다. 정의만 해 두면 죽은 코드다.

> 색 토큰만 바꾸고 활자·여백을 예전대로 두면 "바뀐 게 없다" 는 결과가 된다.
> 실제로 그렇게 두 번 되돌아왔다. 사람이 보는 것은 색이 아니라 크기와 여백이다.

> **AA 미달을 아는 채로 둔 것이 셋 있다.** 강조색 `#ff385c` 는 흰 글자 3.52:1(양쪽),
> `--muted-soft` 는 배경에 따라 라이트 3.11(canvas)~2.90(surface-soft),
> 다크 4.50(canvas)~3.97(surface-soft) — 어느 배경에서도 4.5 를 넘지 못한다.
> 비활성 CTA 는 1.37(WCAG 1.4.3 비활성 예외). 앞의 둘은 원본 문서 값이라 그대로 뒀다.
> 그래서 **버튼 라벨 외의 곳에 `text-primary` 를 쓰지 말 것**이고
> (흰 바탕 위 글자는 `--primary-accent` 6.15:1), **정보를 담은 글자에
> `text-muted-soft` 를 쓰지 말 것**이다 — 플레이스홀더·손잡이 같은 장식용이다.
...
## E2E 는 반드시 로컬 DB 를 쓴다

`playwright.config.ts` 의 webServer env 에 **`MONGODB_URI: ''` 를 값으로 명시**한다.
주석으로 "비워 둔다" 고 적는 것으로는 안 된다.

`next dev` 가 `.env` 를 읽기 때문이다. 개발용 `.env` 에 `MONGODB_URI` 가 생기는
순간 E2E 전체가 그 원격 DB 에 붙어 돈다. 실제로 그렇게 되어 테스트가 운영 Atlas 를
쓰며 계정과 문서를 쌓았고, 매 요청이 네트워크를 타면서 **타임아웃이 테스트마다
옮겨 다니는** 형태로만 드러났다. 드래그 헬퍼를 세 번 고칠 때까지 원인을 못 찾았다.

process.env 가 `.env` 파일보다 우선하므로 빈 문자열을 넣으면 로컬 폴백을 탄다.
E2E 가 느려지거나 실패가 자리를 옮겨 다니면 **먼저 어느 DB 에 붙었는지 확인할 것.**

## E2E 를 돌릴 때 (실제로 당한 것들)

**실행 중에 저장소 안의 파일을 건드리지 말 것.** `next dev` 가 리로드되면서 그 프로세스가
띄운 mongod 가 죽어 `ECONNREFUSED 27017` 로 줄줄이 실패한다. `.omc/` 도 프로젝트
안이라 감시 대상이다. 진행 상황을 기록하고 싶으면 실행이 끝난 뒤에 한다.

**로그를 지운 직후 바로 폴링하지 말 것.** 파일이 다시 만들어지기 전에 `grep EXIT=` 를
돌리면 이전 실행의 결과를 읽고 끝난 줄 안다. 크기가 0이 아닌지 함께 확인한다.

**셋업만 재시도한다.** `loginAs` / `resetData` 의 HTTP 는 일시적 ECONNRESET 에
한 번 다시 시도하고 그 사실을 로그에 남긴다. 단언은 재시도하지 않는다 —
그러면 진짜 결함을 가린다.
