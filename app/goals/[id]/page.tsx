...
export default function GoalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (goalQuery.isPending) {
    return (
      <p data-testid="goal-loading" className="t-body-md text-muted">
        불러오는 중…
      </p>
    )
  }
  ...
  if (goalQuery.isError || !goalQuery.data) {
    return (
      <div className="flex flex-col gap-3">
        <p role="alert" data-testid="goal-error" className="t-body-sm text-danger">
          목표를 불러오지 못했습니다.
        </p>
        <Link href="/goals" className="t-body-sm text-primary-accent underline">
          목록으로
        </Link>
      </div>
    )
  ...
  return (
    <div className="flex flex-col gap-12">
      <header className="flex flex-col gap-2 mb-4">
        <Link href="/goals" data-testid="goal-back" className="t-caption-sm text-muted underline hover:text-ink">
          ← 1년 목표 목록
        </Link>
        <h1 data-testid="goal-detail-title" className="t-display-lg text-ink">
          {goal.title}
        </h1>
        <p className="t-body-md text-muted">
          {goal.year}년 · {formatKstDate(goal.startDate)} ~ {formatKstDate(goal.endDate)}
        </p>
        {goal.description && <p className="t-body-md text-muted">{goal.description}</p>}
      </header>

      <section className="flex flex-col gap-3">
        {/*
          문서는 활자만으로 위계를 세우는 자리를 딱 하나 둔다 — 리스팅의 평점이다.
          이 앱에서 그에 해당하는 것은 목표 진행률이다. 여기서만 크게 간다.
        */}
        <p className="t-rating text-ink">{progress.percent}%</p>
        <ProgressBar percent={progress.percent} testId="goal-detail-bar" showPercent={false} />
        <p data-testid="goal-detail-progress" className="t-caption text-ink">
      ...
      <section className="flex flex-col gap-3">
        <h2 className="t-display-sm text-ink">하위 주간 계획</h2>

        {plans.length === 0 ? (
          <p data-testid="goal-plans-empty" className="t-body-md text-muted">
            연결된 주간 계획이 없습니다.
          </p>
        ) : (
            ...
            {plans.map((entry) => (
              ...
              >
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <Link href={`/week/${formatKstDate(entry.plan.weekStart)}`} className="t-caption text-ink hover:text-primary-accent">
                    {entry.plan.title}
                  </Link>
